...
from lib.models.senet.se_module import SELayer
import torch
...
def se_resnet50(num_classes=1000, pretrained=False, model_path=None):
    """Constructs a ResNet-50 model with Squeeze-and-Excitation blocks.
    ...
    if pretrained and model_path is not None:
        # Load the pre-trained model state dict from a local file
        state_dict = torch.load(model_path)
        
        # Remove the final fully connected layer weights from the state_dict
        ...
        # Redefine the final fully connected layer to match the number of classes
